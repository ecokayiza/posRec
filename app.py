...
########################################################################################################
    ...
    # 各模式处理
    option = st.selectbox('Choose an option', ['Upload Image', 'Upload Video', 'Try existing image','Camera'])
########################################################################################################
    if option == 'Upload Video':
        ...
        if uploaded_video is not None:
            ...
            while success:
                input_image, draw_image, output_scale = process_input(image, scale_factor, output_stride)
                ...
                if result_image is not None:
                    ...
                    if video_display_mode == 'Frame by Frame':
                        video_display.image(result_image, caption=f'Frame {frame_count}', use_container_width=True)
            ...
            if frames:
                ...
                input_image, draw_image, output_scale = process_input(frames[frame_idx], scale_factor, output_stride)
                pose_scores, keypoint_scores, keypoint_coords = run_model(input_image, model, output_stride, output_scale)
...
########################################################################################################    
        ...
        image_dir = st.sidebar.text_input('Image Directory', './images')
        ...
        if filenames:
            selected_name = st.selectbox('Choose an image', filenames)
            
            input_image, source_image, output_scale = posenet.read_imgfile(
                selected_name, scale_factor=scale_factor, output_stride=output_stride)

            filename = os.path.basename(selected_name)
            pose_scores, keypoint_scores, keypoint_coords = run_model(input_image, model, output_stride, output_scale)
            print_frame(source_image, pose_scores, keypoint_scores, keypoint_coords, output_dir, filename=selected_name, min_part_score=min_part_score, min_pose_score=min_pose_score)
...
def print_frame(draw_image, pose_scores, keypoint_scores, keypoint_coords, output_dir, filename=None, min_part_score=0.01, min_pose_score=0.1):
        if output_dir:
            ...
            st.image(draw_image, caption='PoseNet Output', use_container_width=True)
